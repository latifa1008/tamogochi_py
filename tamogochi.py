...
class MiniGame:
    ...
    def update(self):
        # 1. Обновить собаку и игрушки
        self.dog.update()
        self.toys.update()

        # 2. Если выпал 0 из 100, создать игрушку
        if random.randint(0, 100) == 0:
            self.toys.add(Toy())

        # 3. Просчитать столкновения и добавить очки по их количеству
        hits = pg.sprite.spritecollide(self.dog, self.toys, True, pg.sprite.collide_rect_ratio(0.6))
        self.score += len(hits)

        # 4. Если прошло достаточно времени, остановить игру и перейти в основной режим
...
class Dog(pg.sprite.Sprite):
    def __init__(self):
        ...
        # Границы окошка
        self.border_left = 90
        self.border_right = SCREEN_WIDTH - 80
...
class Toy(pg.sprite.Sprite):
    def __init__(self):
        ...
        toy_files = [
            "images/toys/red bone.png",
            "images/toys/blue bone.png",
            "images/toys/ball.png"
        ]

        # Выбираем случайную игрушку
        selected_toy = random.choice(toy_files)

        # Загружаем изображение игрушки
        self.image = load_image(selected_toy, TOY_SIZE, TOY_SIZE)
        self.rect = self.image.get_rect()

        # Спавнимся сверху в случайном месте
    ...
    def update(self):
        ...
        # Если игрушка упала ниже - удаляем её
...
class Game:
    def __init__(self):
        ...
        self.clothes_button = Button("Одежда", button_x, PADDING + ICON_SIZE + ICON_SIZE - 15,
                                     func=self.clothes_menu_on)
        self.games_button = Button("Игры", button_x, PADDING + ICON_SIZE + ICON_SIZE + ICON_SIZE - 30, func=self.game_on)
    ...
    def increase_money(self):
        for cost, is_bought in self.costs_of_upgrade.items():
            if not is_bought and self.money >= cost:
                self.money -= cost
    ...
    def draw(self):
        ...
        self.screen.blit(text_render(self.satiety), (PADDING + ICON_SIZE, PADDING * 23))

        self.screen.blit(self.health_image, (PADDING, PADDING + 80 + 80))
        self.screen.blit(text_render(self.health), (PADDING + ICON_SIZE, PADDING * 39))

        self.screen.blit(self.money_image, (810, PADDING))
        self.screen.blit(text_render(self.money), (775, PADDING * 6))
